...
const StakeAndMintProcessorInterCommKlassSpecificPrototype = {
  ...
  processEventObj: async function(eventObj) {
    ...
    const vcStakeResponse = await openSTValueContractInteract.processStaking(
      stakerAddress,
      stakerPassphrase,
      stakingIntentHash
    );
    ...
    if (vcStakeResponse.isSuccess()) {
      ...
      logger.win(
        stakingIntentHash,
        ':: performed processStaking on openSTValue contract for ' + displayTokenType,
        vcFormattedEvents
      );
    } else {
      // Fire notification event
    ...
    if (ucMintResponse.isSuccess()) {
      ...
      logger.win(
        stakingIntentHash,
        ':: performed processMinting on openSTUtility contract for ' + displayTokenType,
        ucFormattedEvents
      );
    } else {
      // Fire notification event
    ...
    if (ucClaimResponse.isSuccess()) {
      ...
      logger.notify('e_ic_samp_processor_3', stakingIntentHash + ' claim failed for ' + displayTokenType);
...
InstanceComposer.registerShadowableClass(
  StakeAndMintProcessorInterCommKlass,
  'getStakeAndMintProcessorInterCommService'
);
