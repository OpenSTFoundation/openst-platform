...
STPrimeContractKlass.prototype = {
  ...
  perform: async function() {
    ...
    logger.step(
      '** Initialize Transfer of ST Prime - all base tokens from initial ST Prime holder address to ST Prime contract address'
    );
    ...
    if (initialSTPrimeHolderBalanceInWei != stPrimeTotalSupplyInWei) {
      logger.error(
        'Initial ST Prime holder - ' + utilityInitialSTPrimeHolder + " doesn't have max total supply of ST Prime"
      );
      process.exit(1);
    }
    ...
    if (simpleTokenPrimeContractBalanceInWei != stPrimeTotalSupplyInWei) {
      logger.error(
        'simpleTokenPrimeContract: ' + simpleTokenPrimeContractAddress + " doesn't have max total supply of ST Prime"
      );
      process.exit(1);
    }

    const initialSTPrimeHolderBalanceInWeiAfterTransfer = await web3Provider.eth.getBalance(
      coreAddresses.getAddressForUser(utilityInitialSTPrimeHolder)
    );
    ...
    return Promise.resolve(
      responseHelper.successWithData({
        contract: 'stPrime',
        address: simpleTokenPrimeContractAddress,
        uuid: simpleTokenPrimeUUID
      })
    );
  }
};
