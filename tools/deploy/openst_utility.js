...
DeployOpenSTUtilityContractKlass.prototype = {
  ...
  perform: async function(showPrompts) {
    ...
    const openSTUtiltiyContractDeployResponse = await deployHelper.perform(
      openSTUtilityContractName,
      web3Provider,
      openSTUtilityContractAbi,
      openSTUtilityContractBin,
      utilityDeployerName,
      { gasPrice: UC_GAS_PRICE, gas: UC_GAS_LIMIT },
      [VALUE_CHAIN_ID, UTILITY_CHAIN_ID, utilityRegistrarContractAddress]
    );
    ...
    await openStUtility.initiateOwnerShipTransfer(utilityDeployerName, foundationAddress, {
    ...
    return Promise.resolve(
      responseHelper.successWithData({ contract: 'openSTUtility', address: openSTUtilityContractAddress })
    );
  }
};
