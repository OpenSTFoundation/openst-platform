 ...
 */
BrandedToken.prototype.deploy = function(symbol, name, decimals, chainId, deployer) {
    ...
    return Geth.UtilityChain.deployContract(deployer, BrandedTokenContract, oldAddress, symbol, name, decimals, chainId);
};
...
BrandedToken.prototype.mint = function(uuid, minter, minterNonce, amountST, amountUT, escrowUnlockHeight, mintingIntentHash) {
    const REGISTRAR_ADDRESS = coreConstants.OST_REGISTRAR_ADDRESS;
    const REGISTRAR_KEY = coreConstants.OST_REGISTRAR_SECRET_KEY;
    ...
    return Geth.UtilityChain.eth.personal.unlockAccount( REGISTRAR_ADDRESS, REGISTRAR_KEY)
        .then(_ => {
            return this._btInstance.methods.mint(uuid, minter, minterNonce, amountST, amountUT, escrowUnlockHeight, mintingIntentHash).send({from: REGISTRAR_ADDRESS});
        });
};
