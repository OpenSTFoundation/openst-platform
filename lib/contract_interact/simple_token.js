...
const SimpleTokenKlass = function () {
  ...
  simpleTokenContractObj = simpleTokenContractObj || new (web3ProviderFactory.getProvider('value', 'ws')).eth.Contract(simpleTokenContractAbi);

  oThis.contractAddress = simpleTokenContractAddr;
  simpleTokenContractObj.options.address = oThis.contractAddress;


  OpsManagedKlass.call(oThis, oThis.contractAddress, web3ProviderFactory.getProvider('value', 'ws'), simpleTokenContractObj, VC_GAS_PRICE);
};
...
SimpleTokenKlass.prototype.approve = async function (senderAddress, senderPassphrase, spenderAddress, value, inAsync) {
  ...
  const estimateGasResponse = await estimateGasObj.perform()
    ...
    , txParams = {gasPrice: VC_GAS_PRICE, gas: gasToUse}
  ;
...
SimpleTokenKlass.prototype.transfer = async function (senderAddress, senderPassphrase, receiverAddress, amountInWei, options) {
  ...
  const asyncTransfer = async function () {
    const encodedABI = simpleTokenContractObj.methods.transfer(receiverAddress, bigNumAmount.toString(10)).encodeABI();
    ...
    const rawTxParams = {
      from: senderAddress, to: receiverAddress, value: bigNumAmount.toString(10),
    ...
    return new Promise(function (onResolve, onReject) {
      ...
      (web3ProviderFactory.getProvider('value', 'ws')).eth.personal.unlockAccount(senderAddress, senderPassphrase)
        .then(function () {
          return (web3ProviderFactory.getProvider('value', 'ws')).eth.sendTransaction(txParams)
