...
SimpleTokenKlass.prototype.balanceOf = async function (address) {
  const oThis = this
    , callMethodResult = await oThis._callMethod('balanceOf', [address])
 ...
 * @param {number} valueInWeis - OST to transfer in Weis
 *
 * @return {promise}
 *
 */
SimpleTokenKlass.prototype.transfer = async function (senderAddress, senderPassphrase, receiverAddress, valueInWeis, inAsync) {
  const encodedABI = simpleTokenContractObj.methods.transfer(receiverAddress, valueInWeis).encodeABI();
