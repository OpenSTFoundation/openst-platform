...
BrandedTokenKlass.prototype = {
  ...
  /**
   * Get branded token balance without caching
   *
   ...
   *
   */
  getBalanceWithoutCache: function (owner) {
    const oThis = this
    ;
    ...
    if (!basicHelper.isAddressValid(owner)) {
      let errObj = responseHelper.error({
        internal_error_identifier: 'l_ci_bt_getBalanceWithoutCache_1',
        api_error_identifier: 'invalid_address',
        error_config: basicHelper.fetchErrorConfig()
      });
  ...
  getBalanceOf: function (owner) {
    ...
    const callback = async function (response) {
      ...
      if (cacheResult.isSuccess() && cacheResult.data.response != null) {
        ...
        return responseHelper.successWithData({balance: cacheResult.data.response, source: 'cache'});
      }
      //We can't help. throw again.
      if (response.isFailure()) {
        throw response;
      ...
      return responseHelper.successWithData({balance: response.data.balanceOf, source: 'chain'});
    };
  ...
  _creditBalanceInCache: function (owner, bigAmount) {
    ...
    return oThis.getBalanceOf(owner)
      .then(function (response) {
        ...
        // NOTE: No need to credit balance if source was chain(means, just got new balance and had set to cache.)
        if (response.data.source == 'chain') return responseHelper.successWithData({});

        const balance = response.data.balance
          , bigBalance = (new BigNumber(balance)).plus(bigAmount);
        ...
        return oThis.setBalanceToCache(owner, bigBalance)
          .then(function (setResponse) {
            if (setResponse.isSuccess() && setResponse.data.response != null) {
              ...
              logger.error('_creditBalanceInCache :: cache could not be set', setResponse);
              return responseHelper.error({
                internal_error_identifier: 'l_ci_bt_creditBalanceInCache_1',
                api_error_identifier: 'something_went_wrong',
                error_config: basicHelper.fetchErrorConfig()
              });
            }
        ...
        if (response.isSuccess()) {
          ...
          return oThis.setBalanceToCache(owner, bigBalance)
            .then(function (setResponse) {
              ...
              return responseHelper.error({
                internal_error_identifier: 'l_ci_bt_debitBalanceInCache_1',
                api_error_identifier: 'something_went_wrong',
                error_config: basicHelper.fetchErrorConfig()
              });
            });
    ...
    return oThis.getBalanceOf(owner)
      .then(function (response) {
        ...
        if (typeof balance === "undefined" || isNaN(Number(balance))) {
          return responseHelper.error({
            internal_error_identifier: 'l_ci_bt_validateBalance_1',
            api_error_identifier: 'something_went_wrong',
            error_config: basicHelper.fetchErrorConfig()
          });
        }
