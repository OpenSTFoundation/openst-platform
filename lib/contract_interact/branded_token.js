...
const BrandedTokenKlass = function(memberObject) {
  ...
  oThis.currContract = new web3Provider.eth.Contract(
    coreAddresses.getAbiForContract(brandedTokenContractName),
    oThis._getBTAddress()
  );
};
...
BrandedTokenKlass.prototype = {
  ...
  getBalanceOf: async function(owner) {
    ...
    const balanceResponse = await new openSTStorage.cache.TokenBalance({
  ...
  claim: async function(senderAddress, senderPassphrase, beneficiaryAddress) {
    ...
    const claimResponse = await contractInteractHelper.safeSendFromAddr(
      web3Provider,
      oThis._getBTAddress(),
      encodedABI,
      senderAddress,
