 ...
 * @param {string} memberObject.Reserve - Branded token Reserve address
 *
 */
...
BrandedTokenKlass.prototype = {
   ...
   *
   */
  transfer: async function (senderAddr, senderPassphrase, reservePassphrase, recipient, amountInWei, options) {
    ...
    const rollbackFn = function (response, onResolve) {
      ...
      return new Promise(function (onResolve, onReject) {
        web3RpcProvider.eth.personal.unlockAccount(senderAddr, senderPassphrase)
          ...
          .catch(function(reason) {
            logger.error("BT :: Transaction failed.\n\t Reason:", JSON.stringify(reason));
            ...
            // send response
            return rollbackFn(responseHelper.error('l_ci_bt_transfer_6', 'Transaction failed'), onResolve);
          });
      });
  ...
  //     , gasPrice = new BigNumber(UC_GAS_PRICE)
