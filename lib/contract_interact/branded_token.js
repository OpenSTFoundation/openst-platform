...
BrandedTokenContractInteract.prototype = {
  ...
  */
  ...
  _fundSenderForTransferIfNeeded: async function ( transferParams, transactionObject, transactionLogger ) {
    ...
    transactionLogger.logStep("_fundSenderForTransferIfNeeded : estimatedGas = " + estimatedGas);
  ...
  processSafeSendResponse: function ( response, transferParams, transactionLogger ) {
    ...
    if ( !oThis.isSafeSendResponseValid( formattedTxReceipt, transferParams, transactionLogger ) ) {
  ...
  isSafeSendResponseValid: function ( formattedTxReceipt, transferParams, transactionLogger ) {
    const oThis = this;
    ...
    const transferEvent = formattedTxReceipt.eventsData.find( function ( data ) {
      if ( data.name  ) {
        const thisEvent = String( data.name ).toLowerCase();
        if ( thisEvent === expectedEventName ) {
          transactionLogger.logInfo('isSafeSendResponseValid :: found' + expectedEventName, data);
          return data;
        }
      }
      return false;
