...
;
oThis.memberObject = memberObject;
oThis.currContract = new web3Provider.eth.Contract(coreAddresses.getAbiForContract(brandedTokenContractName),
  oThis._getBTAddress());
};
...
getBalanceOf: async function (owner) {
  ...
  let ddbServiceObj = oThis.ic().getDynamoDb();
  // Validate addresses
  if (!basicHelper.isAddressValid(owner)) {
    let errObj = responseHelper.error({
 ...
 *
 */
getBtBalanceFromChain: function (owner) {
  const oThis = this
  ;
  ...
  if (!basicHelper.isAddressValid(owner)) {
    return Promise.resolve(responseHelper.error({
      internal_error_identifier: 'l_ci_bt_getBtBalanceFromChain_1',
      api_error_identifier: 'invalid_address',
      error_config: basicHelper.fetchErrorConfig()
    }));
  }
...
transfer: async function (senderAddr, senderPassphrase, recipient, amountInWei, options) {
  ...
  const asyncTransfer = async function () {
    ...
    const rawTxParams = {
      ...
      gasPrice: coreConstants.OST_UTILITY_GAS_PRICE, gas: gasToUse
    };
...
claim: async function (senderAddress, senderPassphrase, beneficiaryAddress) {
  ...
  const claimResponse = await contractInteractHelper.safeSendFromAddr(web3Provider, oThis._getBTAddress(), encodedABI,
    senderAddress, senderPassphrase, {gasPrice: coreConstants.OST_UTILITY_GAS_PRICE, gas: gasToUse}, addressToNameMap);
  ...
  await oThis._creditBalance(beneficiaryAddress, bigNumAmount);
...
approve: async function (ownerAddress, ownerPassphrase, spenderAddress, value, inAsync) {
  ...
  if (inAsync) {
    ...
    const transactionHash = await contractInteractHelper.sendTxAsyncFromAddr(web3Provider, oThis._getBTAddress(), encodedABI,
      ownerAddress, ownerPassphrase, {gasPrice: coreConstants.OST_UTILITY_GAS_PRICE, gas: gasToUse});
...
_debitBalance: async function (owner, bigAmount) {
  ...
  const oThis = this
    ,  debitAmount = basicHelper.convertToBigNumber(bigAmount).mul(basicHelper.convertToBigNumber(-1))
    , ddbServiceObj = oThis.ic().getDynamoDb();
  ;
...
InstanceComposer.registerShadowableClass(BrandedTokenKlass, "getBrandedTokenInteractClass");
