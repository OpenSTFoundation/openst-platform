...
BrandedTokenKlass.prototype = {
  ...
  getAllowance: function (owner, spender) {
    ...
    if (!contractInteractHelper.isAddressValid(owner)) {
      return Promise.resolve(responseHelper.error('l_ci_getAllowance_1', `Invalid blockchain address: ${owner}`));
    }
    if (!contractInteractHelper.isAddressValid(spender)) {
      return Promise.resolve(responseHelper.error('l_ci_getAllowance_2', `Invalid blockchain address: ${spender}`));
    }
  ...
  getBalanceOf: function (owner) {
    ...
    // Validate addresses
    if (!contractInteractHelper.isAddressValid(owner)) {
      return Promise.resolve(responseHelper.error('l_ci_getBalanceOf_1', `Invalid blockchain address: ${owner}`));
    }
    ...
    return oThis.getBalanceFromCache(owner)
      ...
      .catch(function (err) {
        //Format the error
        logger.error(err);
        return responseHelper.error('l_ci_getBalanceOf_2', 'Something went wrong');
      })
   ...
   *
   */
  transfer: async function (senderAddr, senderPassphrase, recipient, amountInWei, tag) {
    const oThis = this
      , txUUID = uuid.v4();

    //Validate addresses
    if (!contractInteractHelper.isAddressValid(senderAddr)) {
      return Promise.resolve(responseHelper.error('l_ci_bt_t_1', `Invalid blockchain address: ${senderAddr}`));
    }
    if (!contractInteractHelper.isAddressValid(recipient)) {
      return Promise.resolve(responseHelper.error('l_ci_bt_t_2', `Invalid blockchain address: ${recipient}`));
    }
    ...
    if (!bigNumAmount || bigNumAmount.lessThan(1)) {
      return Promise.resolve(responseHelper.error('l_ci_bt_t_3', `Invalid tramsfer amount: ${amountInWei}`));
    }
    ...
    const asyncTransfer = async function() {
      ...
      // Fund ST' if needed
      var fundStPrimeResponse = await oThis._fundSTPrimeIfNeeded(senderAddr, transactionObject);
      if (fundStPrimeResponse.isFailure()) {
        logger.error("BT :: transfer :: failed to fund sender");
        logger.error(fundStPrimeResponse);
        return rollbackFn(fundStPrimeResponse);
      }

      // Transfer branded tokens
      var transferTxHash = await contractInteractHelper.sendTxAsyncFromAddr(web3RpcProvider,  oThis._getBTAddress(),
        transactionObject.encodeABI(), senderAddr, senderPassphrase,
        {
          from: senderAddr,
      ...
      contractInteractHelper.getTxReceipt(web3RpcProvider, transferTxHash, addressToNameMap)
        .then(function (response) {
          if (response.isSuccess()) {
            //Credit the amount to the recipient.
            oThis._creditBalanceInCache(recipient, bigNumAmount);
          } else {
            logger.error('BT :: transfer :: transfer get receipt failed');
  ...
  claim: async function (senderAddress, senderPassphrase, beneficiaryAddress) {
    ...
    return contractInteractHelper.safeSendFromAddr(web3RpcProvider, oThis._getBTAddress(), encodedABI,
      senderAddress, senderPassphrase, {gasPrice: coreConstants.OST_UTILITY_GAS_PRICE});
  },
  ...
  approve: async function (ownerAddress, ownerPassphrase, spenderAddress, value, inAsync) {
    ...
    return contractInteractHelper.sendTxAsyncFromAddr(web3RpcProvider, oThis._getBTAddress(), encodedABI,
      ownerAddress, ownerPassphrase, {gasPrice: coreConstants.OST_UTILITY_GAS_PRICE});
  },
  ...
  /**
   * Transfer ST' (gas) to senderAddr, if needed
   *
   * @param {string} senderAddr - Sender address that need ST'
   * @param {object} transactionObject - transfer transaction object to calculate estimated gas
   *
   * @return {promise<result>}
   *
   * @ignore
   */
  _fundSTPrimeIfNeeded: async function (senderAddr, transactionObject) {
    const oThis = this
      , reserve = oThis._getMemberReserve()
    ;

    // Estimate gas and required cost for transaction
    const estimatedGas = new BigNumber(await transactionObject.estimateGas({from: senderAddr}))
      , gasPrice = new BigNumber(UTILITY_GAS_PRICE)
      , estimatedCost = estimatedGas.mul(gasPrice)
      , stPrimeToTransfer = estimatedCost.times(stPrimeTransferFactor);

    // Get Sender ST' Balance
  ...
  _callMethod: function(methodName, args) {
    const oThis = this
      , mcAddress = oThis._getMemberReserve()
    ...
    return contractInteractHelper.call(web3RpcProvider, btAddress, encodeABI, {from: mcAddress}, transactionOutputs)
