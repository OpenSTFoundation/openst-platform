...
BrandedTokenKlass.prototype = {
  ...
  getBalanceWithoutCache: function (owner) {
    ...
    // Validate addresses
    if (!basicHelper.isAddressValid(owner)) {
      return Promise.resolve(responseHelper.error('l_ci_bt_getBalanceWithoutCache_1', `Invalid blockchain address: ${owner}`));
    }
  ...
  approve: async function (ownerAddress, ownerPassphrase, spenderAddress, value, inAsync) {
    ...
    if (inAsync) {
      return contractInteractHelper.sendTxAsyncFromAddr(web3Provider, oThis._getBTAddress(), encodedABI,
