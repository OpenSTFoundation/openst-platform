...
OpenSTValueKlass.prototype.utilityTokens = async function(uuid) {
  ...
  if (response.length != 8) {
    ...
    if (response[0] != '') {
      const conversionFactorResponse = basicHelper.convertConversionRateToConversionFactor(
        conversionRate,
 ...
 *
 */
OpenSTValueKlass.prototype.stake = async function(
  senderAddr,
  senderPassphrase,
  uuid,
  amountSTWeis,
 ...
 * deactivate contract
 *
 * @param {string} senderAddr - address which will sign this transaction
 * @param {string} senderPassphrase - passphrase
 *
 * @return {promise}
 */
OpenSTValueKlass.prototype.deactivate = async function (senderAddr, senderPassphrase) {

  const oThis = this,
    encodedABI = oThis.openSTValueContractObj.methods.deactivate().encodeABI(),
    web3ProviderFactory = oThis.ic().getWeb3ProviderFactory(),
    contractInteractHelper = oThis.ic().getContractInteractHelper(),
    web3Provider = web3ProviderFactory.getProvider('value', 'ws')
  ;
  ...
  logger.debug('deactivate OpenStValue -------------------------------------------------------');
  logger.debug(JSON.stringify(transactionReceiptResult));
  logger.debug('-------------------------------------------------------');

  return Promise.resolve(responseHelper.successWithData({
    transactionReceipt: transactionReceiptResult
  }));
 ...
 * @return {promise}
 */
OpenSTValueKlass.prototype.isDeactivated = async function () {

  const oThis = this
    , callMethodResult = await oThis._callMethod('deactivated')
  ;

  return Promise.resolve(callMethodResult);
