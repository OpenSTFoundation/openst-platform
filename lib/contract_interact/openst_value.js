...
const contractName = 'openSTValue'
 ...
 *
 */
OpenSTValue.prototype.stake = async function (reserveAddress,
                                              reservePassphrase,
                                              uuid,
                                              amountST,
                                              beneficiaryAddr,
...
OpenSTValue.prototype.getRegistrar = function () {
  return currContract.methods.registrar().call()
    .then(_out => {
      logger.info("getRegistrar :: _out", _out);
 ...
 *
 */
OpenSTValue.prototype.utilityTokenProperties = async function (uuid) {
  const transactionObject = currContract.methods.utilityTokenProperties(uuid);
...
OpenSTValue.prototype.getHashUUID = function (sym, name, chainIdValue, chainIdUtility, contractAddress, convertionRate) {
  return currContract.methods.hashUuid(sym, name, chainIdValue, chainIdUtility, contractAddress, convertionRate).call()
    .then(function (_out) {
      logger.info("getHashUUID :: _out", _out);
      return Promise.resolve(_out);
    })
    ;
};
