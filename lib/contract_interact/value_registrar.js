...
const ValueRegistrarKlass = function(contractAddress) {
  ...
  oThis.valueRegistrarContractObj = valueRegistrarContractObj;
...
ValueRegistrarKlass.prototype.valueRegistrarContractObj = null;
...
ValueRegistrarKlass.prototype.addCore = function(senderName, registry, coreContractAddress) {
  ...
  const encodedABI = oThis.valueRegistrarContractObj.methods.addCore(registry, coreContractAddress).encodeABI();

  return contractInteractHelper.safeSend(web3Provider, this.contractAddress, encodedABI, senderName, {
    gasPrice: oThis.VC_GAS_PRICE,
    gas: oThis.VC_GAS_LIMIT
  });
};
 ...
 *
 */
ValueRegistrarKlass.prototype.registerUtilityToken = async function(
  senderAddress,
  senderPassphrase,
  registry,
  symbol,
  ...
  const gasToUse = await oThis.valueRegistrarContractObj.methods
  ...
  const encodedABI = oThis.valueRegistrarContractObj.methods
    .registerUtilityToken(
      registry,
      symbol,
      name,
 ...
 *
 */
ValueRegistrarKlass.prototype.confirmRedemptionIntent = function(
  senderAddress,
  senderPassphrase,
 ...
 *
 */
ValueRegistrarKlass.prototype.processStaking = function (senderAddress, senderPassphrase, stakingIntentHash) {
  ...
  const encodedABI = oThis.valueRegistrarContractObj.methods.processStaking(openSTValueContractAddress, stakingIntentHash).encodeABI();
