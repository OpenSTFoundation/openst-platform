...
ContractInteractHelperKlass.prototype = {
   ...
   *
   */
  safeSendFromAddr: function (web3Provider, currContractAddr, encodeABI, senderAddr, senderPassphrase, txOptions,
                              addressToNameMap) {
    const oThis = this
    ;

    return oThis.sendTxAsyncFromAddr(web3Provider, currContractAddr, encodeABI, senderAddr, senderPassphrase,
   ...
   * @ignore
   */
  sendTxAsyncFromAddr: function (web3Provider, currContractAddr, encodeABI, senderAddr, senderPassphrase,
                                 txOptions) {
    const txParams = {
      from: senderAddr,
      to: currContractAddr,
    ...
    return new Promise(function(onResolve, onReject) {
      ...
      var getReceipt = async function() {
        if (maxAttempts > 0) {
          ...
          return onResolve(responseHelper.error("l_ci_h_getTransactionReceiptFromTrasactionHash", errorReason));
        }
      };
