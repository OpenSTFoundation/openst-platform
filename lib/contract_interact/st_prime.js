...
StPrimeKlass.prototype = {
   ...
   * @param {boolean} [options.inAsync] - 'false' if you want to wait till TX mining. 'true' will only send txUUID. Default: true
   *
   * @return {promise<result>}
   *
   */
  transfer: async function (senderAddr, senderPassphrase, recipient, amountInWei, options) {
    const oThis = this
      ...
      , tag = options.tag
      , inAsync = (options.inAsync === false) ? false : true
    ;
    ...
    const asyncTransfer = async function() {
      ...
      return web3RpcProvider.eth.personal.unlockAccount(senderAddr, senderPassphrase)
        ...
        .then(function(transactionHash) {
          ...
          return responseHelper.successWithData({txUuid: txUUID, transactionHash: transactionHash, tag: tag});
        })
