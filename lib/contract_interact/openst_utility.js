...
var openSTUtilityContractObj = null;
/**
...
const OpenStUtilityKlass = function(contractAddress) {
  ...
  oThis.openSTUtilityContractObj = openSTUtilityContractObj;
...
OpenStUtilityKlass.prototype.openSTUtilityContractObj = null;
 ...
 *
 */
OpenStUtilityKlass.prototype.proposeBrandedToken = async function(
  senderAddress,
  senderPassphrase,
  symbol,
  name,
  ...
  var gasToUse = await oThis.openSTUtilityContractObj.methods
    .proposeBrandedToken(symbol, name, conversionRate, conversionRateDecimals)
    .estimateGas({
      from: senderAddress,
      gasPrice: oThis.UC_GAS_PRICE,
      gas: oThis.UC_GAS_LIMIT
    });

  const encodedABI = oThis.openSTUtilityContractObj.methods
    .proposeBrandedToken(symbol, name, conversionRate, conversionRateDecimals)
    .encodeABI();
 ...
 *
 */
OpenStUtilityKlass.prototype.redeem = async function(
  senderAddress,
  senderPassphrase,
  uuid,
  amountBT,
  ...
  const gasToUse = await oThis.openSTUtilityContractObj.methods
    .redeem(uuid, amountBT, nonce, beneficiaryAddr)
    .estimateGas({
      from: senderAddress,
      gasPrice: oThis.UC_GAS_PRICE,
      gas: oThis.UC_GAS_LIMIT
  ...
  const encodedABI = oThis.openSTUtilityContractObj.methods.redeem(uuid, amountBT, nonce, beneficiaryAddr).encodeABI();
 ...
 *
 */
OpenStUtilityKlass.prototype.redeemSTPrime = async function(
  senderAddress,
  senderPassphrase,
  amountSTPrime,
  nonce,
  ...
  //Calculate gas required
  const gasToUse = await oThis.openSTUtilityContractObj.methods.redeemSTPrime(nonce, beneficiaryAddr).estimateGas({
    from: senderAddress,
    gasPrice: oThis.UC_GAS_PRICE,
  ...
  const encodedABI = oThis.openSTUtilityContractObj.methods.redeemSTPrime(nonce, beneficiaryAddr).encodeABI();
 ...
 *
 */
OpenStUtilityKlass.prototype.processRedeeming = async function(
  redeemerAddress,
  redeemerPassphrase,
  redeemptionIntentHash
) {
  const oThis = this,
    web3ProviderFactory = oThis.ic().getWeb3ProviderFactory(),
    contractInteractHelper = oThis.ic().getContractInteractHelper(),
  ...
  const encodedABI = oThis.openSTUtilityContractObj.methods.processRedeeming(redeemptionIntentHash).encodeABI();
...
OpenStUtilityKlass.prototype.processMinting = async function(senderAddress, senderPassphrase, stakingIntentHash) {
  const oThis = this,
    ...
    web3Provider = web3ProviderFactory.getProvider('utility', 'ws'),
    encodedABI = oThis.openSTUtilityContractObj.methods.processMinting(stakingIntentHash).encodeABI(),
...
OpenStUtilityKlass.prototype._callMethod = function(methodName, args) {
  const oThis = this,
    ...
    contractInteractHelper = oThis.ic().getContractInteractHelper(),
    web3Provider = web3ProviderFactory.getProvider('utility', 'ws'),
    scope = oThis.openSTUtilityContractObj.methods,
  ...
  return contractInteractHelper
    .call(web3ProviderFactory.getProvider('utility', 'ws'), oThis.contractAddress, encodeABI, {}, transactionOutputs)
...
InstanceComposer.registerShadowableClass(OpenStUtilityKlass, 'getOpenSTUtilityInteractClass');
