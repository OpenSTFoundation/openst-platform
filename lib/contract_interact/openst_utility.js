...
OpenStUtilityContractInteract.prototype.redeem = async function (senderAddress,
  ...
  const gasToUse = await currContract.methods.redeem(uuid, amountBT, nonce, beneficiaryAddr).estimateGas({
    from: senderAddress,
    gasPrice: UC_GAS_PRICE
  });

  const encodedABI = currContract.methods.redeem(uuid, amountBT, nonce, beneficiaryAddr).encodeABI();
  logger.info("redeem :: \n\tgasToUse", gasToUse,
    "\n\tuuid", uuid,
    "\n\tamountBT", amountBT,
    "\n\tnonce", nonce,
...
OpenStUtilityContractInteract.prototype.redeemSTPrime = async function (senderAddress,
  ...
  //Calculate gas required
  const gasToUse = await currContract.methods.redeemSTPrime(nonce, beneficiaryAddr).estimateGas({
    from: senderAddress,
    gasPrice: UC_GAS_PRICE,
  ...
  const encodedABI = currContract.methods.redeemSTPrime(nonce, beneficiaryAddr).encodeABI();
