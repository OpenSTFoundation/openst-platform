...
const OpenStUtilityKlass = function (contractAddress) {
  ...
  openSTUtilityContractObj = openSTUtilityContractObj || new (web3ProviderFactory.getProvider('utility','ws')).eth.Contract(openSTUtilityContractAbi);
  ...
  OwnedKlass.call(oThis, contractAddress, web3ProviderFactory.getProvider('utility', 'ws'), openSTUtilityContractObj, UC_GAS_PRICE)
};
...
OpenStUtilityKlass.prototype.redeemSTPrime = async function (senderAddress, senderPassphrase,
  ...
  const web3Provider = web3ProviderFactory.getProvider('utility', 'ws')
    , sanitizedAmount = web3Provider.utils.toHex(web3Provider.utils.toWei(amountSTPrime.toString()));
...
OpenStUtilityKlass.prototype.processMinting = async function (senderAddress, senderPassphrase, stakingIntentHash) {
  ...
  const oThis = this
    ...
    , currentGasPrice = new BigNumber(await (web3ProviderFactory.getProvider('utility', 'ws')).eth.getGasPrice())
  ;
