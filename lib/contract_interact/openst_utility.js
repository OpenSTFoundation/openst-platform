...
OpenStUtilityKlass.prototype.proposeBrandedToken = async function (senderAddress,
  ...
  var gasToUse = await oThis.openSTUtilityContractObj.methods.proposeBrandedToken(symbol, name, conversionRate, conversionRateDecimals).estimateGas({
    from: senderAddress, gasPrice: oThis.UC_GAS_PRICE, gas: oThis.UC_GAS_LIMIT
  });
  
  const encodedABI = oThis.openSTUtilityContractObj.methods.proposeBrandedToken(symbol, name, conversionRate, conversionRateDecimals).encodeABI();
...
OpenStUtilityKlass.prototype.redeem = async function (senderAddress, senderPassphrase, uuid,
  ...
  const gasToUse = await oThis.openSTUtilityContractObj.methods.redeem(uuid, amountBT, nonce, beneficiaryAddr).estimateGas({
    from: senderAddress, gasPrice: oThis.UC_GAS_PRICE, gas: oThis.UC_GAS_LIMIT
  });
...
OpenStUtilityKlass.prototype._callMethod = function (methodName, args) {
  ...
  return contractInteractHelper.call(web3ProviderFactory.getProvider('utility', 'ws'), oThis.contractAddress, encodeABI, {}, transactionOutputs)
