...
web3EventsDecoder.prototype = {
  ...
  perform: function (txReceipt, addressToNameMap) {
    ...
    // Transaction failed
    if (txReceipt.status && parseInt(txReceipt.status, 16) !== 1) {
      logger.error("Transaction failed as status is ", txReceipt);
      return responseHelper.error('tx_receipt_status_failed', 'Transaction receipt failed.');
    }
