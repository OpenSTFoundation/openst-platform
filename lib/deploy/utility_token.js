...
const performer = async function(member) {
  ...
  console.log("Calling method for getting UUID");
  console.log("Contract Address for member: " + member.Name + 'is: ' + contractDeployTxReceipt.contractAddress);
  var response = await utilityTokenContractInteract.getUuid(member.Reserve, contractDeployTxReceipt.contractAddress);
  console.log("UUID for member: " + member.Name + 'is: ' + response.data.uuid);
